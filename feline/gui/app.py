...
def run_gui():
 ...
 class Window(QMainWindow):
  def __init__(self):
   super().__init__();self.setWindowTitle("Feline Exchange v0.14.0 — AI-Assisted Paper Workstation");self.settings=QSettings("FelineExchange","Workstation");self.resize(self.settings.value("size",self.size()));self.chart_data={};self.events=EventProjection();self.controller=WorkstationController();self.dataset=None;self.instrument=None;self.chart_marker_items=[];self.displayed_latest={};self.pending_follow_steps=0;self.chart_initialized=False
   ...
   center=QWidget();cl=QVBoxLayout(center);controls=QHBoxLayout();self.open=QPushButton("Open Dataset");self.live=QPushButton("Start OANDA Paper");self.speed=QComboBox();self.speed.addItems(["0.25","0.5","1","2","5","10","MAX"]);self.chart_mode=QComboBox();self.chart_mode.addItems(["Candles","Line"]);self.timeframe=QComboBox();self.timeframe.addItems(["1m","5m","15m","1h"]);self.auto_follow=QCheckBox("Auto-fit && Follow");self.auto_follow.setChecked(True);self.play=QPushButton("Start");self.pause=QPushButton("Pause");self.stop=QPushButton("Stop");self.fit=QPushButton("Fit");self.export=QPushButton("Export Replay Report");self.export.setEnabled(False);[controls.addWidget(x) for x in (self.open,self.live,self.speed,self.chart_mode,self.timeframe,self.auto_follow,self.play,self.pause,self.stop,self.fit,self.export)];cl.addLayout(controls);self.plot=pg.PlotWidget(axisItems={"bottom":pg.DateAxisItem()});self.plot.showGrid(x=True,y=True,alpha=.2);self.curve=self.plot.plot(pen=pg.mkPen("#55b7ff",width=2));self.candle_item=CandlestickItem();self.plot.addItem(self.candle_item);cl.addWidget(self.plot);split.addWidget(center)
  ...
  def start_live(self):
   try:self.controller.start_realtime((self.instrument or "EURUSD",))
   except Exception as exc:QMessageBox.warning(self,"OANDA realtime paper",str(exc))
  ...
  def refresh(self):
   ...
   if not snap:return
   p=snap["portfolio"];r=snap["risk"];a=snap["ai"];sid=(snap.get("replay_session_id") or "--------")[:8];feed=snap.get("feed",{});self.status.setText(f"SESSION {sid}   RUNTIME ● {self.controller.replay.state.value.upper()}   FEED ● {feed.get('state','SYNTHETIC')}   DB ● OK   AI ● {'OK' if a['available'] else 'UNAVAILABLE'}   DANGER ● {'ON' if r['danger'] else 'OFF'}   KILL ● {'ON' if r['kill_switch'] else 'OFF'}")
   if feed:self.right["Provider"].setPlainText(f"Provider: {feed.get('provider')}\nState: {feed.get('state')}\nLast source: {feed.get('last_source_timestamp')}\nLast ingestion: {feed.get('last_ingestion_timestamp')}\nActive instrument: {self.instrument or '—'}")
   self.right["Portfolio"].setPlainText("\n".join(f"{k}: {v:,.4f}" for k,v in p.items() if isinstance(v,(int,float))));self.right["Risk"].setPlainText("\n".join(f"{k}: {v}" for k,v in r.items()));latest=a.get("latest") or {};self.right["AI Opinion"].setPlainText(f"AI REASONING — {a.get('decision_mode')}\nProvider: {a.get('provider')}\nModel: {a['model']}\nStatus: {a.get('status')}\nQueue: {a['queue']}\nAssessment: {latest.get('suggested_action','—')}\nConfidence: {latest.get('confidence','—')}\nLatency: {latest.get('latency_ms','—')} ms\nReasoning: {latest.get('reasoning_summary') or latest.get('summary','—')}\nDownstream: {latest.get('downstream_decision','—')}\nVetoed: {latest.get('vetoed','—')}")
