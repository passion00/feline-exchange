...
from feline.core.events import CandleUpdate,Event,FeedHealthEvent,MarketThesis,NewsEvent,PriceTick,SignalEvent,RiskEvent,OrderUpdate,FillEvent,AIAnalysisResult,RegimeEvent,EconomicEvent,ThesisStateEvent
...
def projection_sort_key(row):
 value=row.get("timestamp")
 try:stamp=float(value) if isinstance(value,(int,float)) else datetime.fromisoformat(str(value).replace("Z","+00:00")).timestamp()
 except Exception:stamp=float("inf")
 return (stamp,row.get("display_order",2),row.get("projection_sequence",0))
...
class EventProjection:
 def __init__(self,limit=1000):self.rows=deque(maxlen=limit);self.sequence=0
 def add(self,timestamp,category,description,instrument=None,details=None):
  self.sequence+=1;rank=0 if description=="EconomicEvent" else 1 if category=="MACRO" else 2;self.rows.append({"timestamp":timestamp,"category":category,"instrument":instrument,"description":description,"details":details or {},"display_order":rank,"projection_sequence":self.sequence})
...
class WorkstationController:
 ...
 async def _event(self,event):
  ...
  if isinstance(event,NewsEvent):self._emit({"kind":"news","timestamp":event.timestamp.isoformat(),"source":event.source,"headline":event.headline,"state":"INGESTED","importance":None,"payload":event.payload()});return
  if isinstance(event,MarketThesis):self._emit({"kind":"thesis","timestamp":event.timestamp.isoformat(),"thesis":event.payload()});return
  if isinstance(event,ThesisStateEvent):self._emit({"kind":"thesis_state","timestamp":event.timestamp.isoformat(),"lifecycle":event.payload()});return
 ...
 async def _run_mixed(self,path,speed):
  ...
  for index,event in enumerate(events):
   ...
   if isinstance(event,NormalizedEconomicEvent):
    ...
    self.runtime.submit_news(event);row={"kind":"news","timestamp":timestamp.isoformat(),"source_timestamp":timestamp.isoformat(),"headline":event.headline,"source":event.source,"state":"QUEUED"};self._record("news",row);self._emit(row);continue
   else:
