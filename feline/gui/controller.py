...
class WorkstationController:
 ...
 def snapshot(self):
  ...
  session_id=self.session["replay_session_id"] if self.session else self.runtime.realtime_session_id;p=self.runtime.broker.portfolio_state();latest=self.runtime.latest_ai.payload() if self.runtime.latest_ai else None;return {"portfolio":p,"risk":{"trading_enabled":self.runtime.risk.trading_enabled,"kill_switch":self.runtime.risk.kill_switch,"danger":self.runtime.risk.danger.active(),"daily_pnl":self.runtime.risk.state.daily_pnl,"max_exposure":self.runtime.config.risk.max_total_exposure},"prices":{k:{"bid":v.bid,"ask":v.ask,"mid":v.mid,"spread":v.spread_ratio,"regime":self.runtime.regimes.current.get(k).value if k in self.runtime.regimes.current else "unknown"} for k,v in self.runtime.broker.quotes.items()},"ai":{"queue":self.runtime.ai.queue.qsize(),"available":self.runtime.ai.last_available,"provider":self.runtime.config.ai.provider,"model":self.runtime.config.ai.model,"decision_mode":self.runtime.config.ai.decision_mode,"status":"disabled" if not self.runtime.config.ai.enabled else "available" if self.runtime.ai.last_available else "unavailable" if self.runtime.ai.last_available is False else "waiting","latest":latest},"positions":[vars(x) for x in self.runtime.broker.positions.values()],"orders":[{**x.payload(),"replay_session_id":session_id} for x in self.runtime.broker.orders.values()],"fills":[{**x.payload(),"replay_session_id":session_id} for x in self.runtime.broker.fills[-100:]],"trades":[{**vars(x),"replay_session_id":session_id} for x in self.runtime.trades.completed],"macro":vars(self.macro) if self.macro else None,"phase":self.phase,"shock":self.shock.value,"strategy":self.strategy,"continuous":self.continuous,"horizons":self.horizons,"abstentions":self.abstentions,"replay_session_id":session_id,"realtime_session_id":self.runtime.realtime_session_id,"feed":self.feed,"mode":self.mode,"dropped":self.dropped}
