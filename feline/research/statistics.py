...
def aggregate_results(results,bootstrap_samples=500,seed=0):
 ...
 return {"counts":{"total":len(results),"included":len(included),"excluded":len(results)-len(included),"FOMC":sum(v for k,v in banks.items() if "FED" in k or "FOMC" in k),"ECB":sum(v for k,v in banks.items() if "ECB" in k)},"strategy_outcomes":dict(outcomes),"directions":dict(directions),"central_banks":dict(banks),"horizons":horizon_stats,"average_shock_magnitude":statistics.fmean(shocks) if shocks else None,"median_shock_magnitude":statistics.median(shocks) if shocks else None,"stabilization_seconds":describe(stabilization),"no_trade_frequency":outcomes.get("NO_TRADE",0)/len(included) if included else None,"no_trade_reasons":dict(no_trade_reasons),"missed_move_candidates":missed}
