...
def run_continuous_experiment(dataset:Path,instrument:str="EURUSD",strategy:str="all",seed:int=0,no_trades:bool=False,output_root:Path=Path("data/reports/continuous"),config:ContinuousConfig|None=None)->dict[str,Any]:
    ...
    for index,candle in enumerate(candles):
        ...
        if no_trades:paper.update_quote(quote)
            ...
            if open_trade and (not position or not position.quantity):
                trades.append(completed_trade(open_trade,paper.fills[-1],"protective_exit",candles[open_trade["entry_index"]:index+1]));open_trade=None
        ...
        if open_trade and (index-open_trade["entry_index"]>=15 or regime.regime is ContinuousRegime.EVENT_RISK):
            ...
            if approval.approved and paper.fills:
                trades.append(completed_trade(open_trade,paper.fills[-1],"event_risk" if regime.regime is ContinuousRegime.EVENT_RISK else "time_exit",candles[open_trade["entry_index"]:index+1]));open_trade=None
    ...
    if open_trade and candles:
        ...
        if approval.approved and paper.fills:
            trades.append(completed_trade(open_trade,paper.fills[-1],"replay_end",candles[open_trade["entry_index"]:]))
    ...
    strategies={family.value:family_metrics(family) for family in StrategyFamily if family is not StrategyFamily.NONE};portfolio={"reference_gross_pnl":sum(trade["reference_gross_pnl"] for trade in trades),"execution_pnl":sum(trade["execution_pnl"] for trade in trades),"gross_pnl":sum(trade["gross_pnl"] for trade in trades),"spread_costs":sum(trade["spread_costs"] for trade in trades),"slippage_costs":sum(trade["slippage_costs"] for trade in trades),"commissions":sum(trade["commissions"] for trade in trades),"financing_costs":sum(trade["financing_costs"] for trade in trades),"net_pnl":sum(trade["net_pnl"] for trade in trades),"transaction_costs":sum(trade["spread_costs"]+trade["slippage_costs"]+trade["commissions"]+trade["financing_costs"] for trade in trades),"max_drawdown":None};
