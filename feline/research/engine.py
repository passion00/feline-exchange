...
from .postshock import calculate_post_shock
...
def _event_result(episode,split,report,records,policy,missed_threshold,flat_tolerance=.001,classification_minutes=15):
 ...
 bucket="small" if abs(shock)<.002 else "medium" if abs(shock)<.005 else "large";stable_time=datetime.fromisoformat(stable["source_timestamp"]) if stable else None;post_shock=calculate_post_shock(market,scheduled,stable_time,episode.entry.secondary_events,policy,flat_tolerance,classification_minutes)
 for minutes,value in post_shock.get("incremental_horizons",{}).items():post_shock[f"return_1m_to_{minutes}m"]=value["return_value"]
 for minutes,value in post_shock.get("post_stabilization_horizons",{}).items():post_shock[f"stabilization_to_{minutes}m_return"]=value["return_value"]
 return {"schema_version":RESEARCH_SCHEMA,"status":"included","event_id":event.event_id,"episode_id":episode.episode_id,"replay_session_id":report["metadata"]["replay_session_id"],"central_bank":event.central_bank,"event_type":event.event_type,"instrument":event.instruments[0],"year":event.scheduled_timestamp.year,"split":split,"primary_timestamp":event.scheduled_timestamp.isoformat(),"dataset_checksum":episode.dataset.checksum,"quality_flags":list(episode.quality_flags),"pre_event_price":baseline,"announcement_candle":post[0] if post else None,"pre_event_volatility":volatility(pre),"post_event_volatility":volatility(post),"spread_assumption":episode.dataset.spread_provenance,"shock_direction":"up" if shock>0 else "down" if shock<0 else "neutral","shock_magnitude":shock,"shock_magnitude_bucket":bucket,"shock_detection_time":shock_row.get("source_timestamp") if shock_row else None,"stabilization_time":stable.get("source_timestamp") if stable else None,"stabilization_duration_seconds":(stable_time-scheduled).total_seconds() if stable_time else None,"strategy_outcome":strategy.get("state","NO_TRADE"),"direction":strategy.get("direction","neutral"),"confidence":strategy.get("confidence",0),"no_trade_reason":strategy.get("reason") if strategy.get("state")=="NO_TRADE" else None,"decision_diagnostics":strategy.get("decision_diagnostics",[]),"horizons":horizons,"maximum_move":max((x["mfe"] for x in horizons.values()),default=None),"reversal_magnitude":min((x["mae"] for x in horizons.values()),default=None),"missed_move_candidate":strategy.get("state")=="NO_TRADE" and later>=missed_threshold,"secondary_events":[asdict(x) for x in episode.entry.secondary_events],**post_shock}
...
def export_experiment(directory:Path,experiment:dict,results:list,aggregate:dict,exclusions:list)->None:
 ...
 event_fields=["event_id","central_bank","event_type","instrument","year","split","primary_timestamp","strategy_outcome","direction","confidence","no_trade_reason","decision_diagnostics","shock_magnitude","stabilization_time","stabilization_duration_seconds","stabilization_price","post_stabilization_outcome","return_1m_to_5m","return_1m_to_15m","return_1m_to_30m","return_1m_to_60m","stabilization_to_5m_return","stabilization_to_15m_return","stabilization_to_30m_return","stabilization_to_60m_return","retracement_fraction","impulse_retention_fraction","maximum_post_stabilization_extension","maximum_post_stabilization_reversal","time_to_post_event_high_seconds","time_to_post_event_low_seconds","missed_move_candidate","quality_flags","replay_session_id"]
 with (directory/"events.csv").open("w",newline="",encoding="utf-8") as handle:
  writer=csv.DictWriter(handle,fieldnames=event_fields);writer.writeheader()
  for row in results:
   if row["status"]!="included":continue
   exported={**row,"stabilization_price":(row.get("stabilization_reference") or {}).get("price")};writer.writerow({k:json.dumps(exported[k]) if isinstance(exported.get(k),(list,dict)) else exported.get(k) for k in event_fields})
 with (directory/"horizons.csv").open("w",newline="",encoding="utf-8") as handle:
  fields=["event_id","split","reference_basis","horizon_minutes","reference_timestamp","target_timestamp","return_value","mae","mfe","volatility","classification","contamination_status","initial_shock_relation"];writer=csv.DictWriter(handle,fieldnames=fields);writer.writeheader()
  for row in results:
   for horizon,value in row.get("horizons",{}).items():writer.writerow({"event_id":row["event_id"],"split":row.get("split"),"reference_basis":"announcement","horizon_minutes":horizon,**{x:value.get(x) for x in fields[4:]}})
   for horizon,value in row.get("incremental_horizons",{}).items():writer.writerow({"event_id":row["event_id"],"split":row.get("split"),"reference_basis":"one_minute","horizon_minutes":horizon,**{x:value.get(x) for x in fields[4:]}})
   for horizon,value in row.get("post_stabilization_horizons",{}).items():writer.writerow({"event_id":row["event_id"],"split":row.get("split"),"reference_basis":"stabilization","horizon_minutes":horizon,**{x:value.get(x) for x in fields[4:]},"contamination_status":value.get("status")})
 ...
 post=aggregate["post_stabilization_outcomes"];post_lines=", ".join(f"{h}m mean={v['mean']} clean={v['clean']} contaminated={v['contaminated']}" for h,v in aggregate["post_stabilization_horizons"].items())
 (directory/"summary.md").write_text(f"# Historical macro research summary\n\n- Experiment: `{experiment['experiment_id']}`\n- Events: {aggregate['counts']['included']} included / {aggregate['counts']['total']} total\n- Excluded: {aggregate['counts']['excluded']} — {reasons}\n- Central banks: {banks or 'none'}\n- Years: {years}\n\n## Announcement-anchored behavior\n\n- CONTINUATION: {out.get('CONTINUATION',0)}\n- MEAN_REVERSION: {out.get('MEAN_REVERSION',0)}\n- NO_TRADE: {out.get('NO_TRADE',0)}\n- NO_TRADE reasons: {aggregate['no_trade_reasons']}\n- Missed-move candidates after abstention: {aggregate['missed_move_candidates']}\n- Largest absolute mean observed return: {strong[0]}m ({strong[1]})\n- Contaminated announcement measurements: {contaminated}\n\n## Post-stabilization behavior\n\n- Stabilization detected: {aggregate['with_stabilization']}\n- No stabilization: {aggregate['without_stabilization']}\n- CONTINUATION: {post.get('CONTINUATION',0)}\n- MEAN_REVERSION: {post.get('MEAN_REVERSION',0)}\n- FLAT: {post.get('FLAT',0)}\n- NO_STABILIZATION: {post.get('NO_STABILIZATION',0)}\n- Median stabilization seconds: {aggregate['stabilization_seconds']['median']}\n- Median retracement: {aggregate['retracement_fraction']['median']}\n- Median impulse retention: {aggregate['impulse_retention_fraction']['median']}\n- Horizon behavior: {post_lines}\n\nClassification statistics do not establish profitability. Contaminated horizons, synthetic execution assumptions, small samples, and selection effects must be reviewed.\n",encoding="utf-8")
...
def run_experiment(manifest_path:Path,config:AppConfig,output_root:Path=Path("data/reports/research"),fail_fast=False,progress=None,cancel_event=None)->dict:
 ...
 with tempfile.TemporaryDirectory(prefix="feline-research-") as tmp:
  for index,episode in enumerate(episodes):
   ...
   controller=None
   try:
    path=_episode_file(episode,Path(tmp));controller=WorkstationController(config);controller.start_replay(str(path),"MAX",manifest.seed);controller.future.result();report=controller.build_report();row=_event_result(episode,split,report,controller.records,manifest.contamination_policy,manifest.missed_move_threshold,manifest.post_stabilization_flat_tolerance,manifest.post_stabilization_classification_minutes);db.save_research_episode(experiment_id,episode,row,split,"completed",row["replay_session_id"]);db.save_post_shock_metrics(experiment_id,event_id,row);results.append(row)
 ...
 if progress:progress({"experiment_id":experiment_id,"completed":len(results),"total":len(episodes),"failed":len(exclusions),"state":experiment["status"],"post_stabilization":aggregate.get("post_stabilization_outcomes",{}),"median_stabilization_seconds":aggregate.get("stabilization_seconds",{}).get("median"),"median_retracement":aggregate.get("retracement_fraction",{}).get("median"),"median_impulse_retention":aggregate.get("impulse_retention_fraction",{}).get("median"),"post_stabilization_horizons":aggregate.get("post_stabilization_horizons",{})})
