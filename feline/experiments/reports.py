...
def compare_reports(paths: list[Path], output: Path | None = None) -> dict:
    ...
    for report in reports:
        s = report["summary"]
        rows.append({"experiment_id": report["metadata"]["experiment_id"], "provider": report["metadata"]["ai_provider"], "model": report["metadata"]["model"], "cases": len(report["cases"]),"schema_valid_responses":s["performance"].get("successful_responses"), "schema_failures":s["engineering"]["schema_failures"],"timeouts":s["performance"].get("timeouts"),"not_evaluated":s["ai_quality"].get("not_evaluated"), "mean_semantic_score": s["ai_quality"]["mean_semantic_score"], "false_positive_rate": s["relevance"]["irrelevant_false_positive_rate"], "abstention_rate": s["relevance"]["irrelevant_abstention_rate"], "unsupported_proposals": s["instrument_quality"]["unsupported_instrument_proposals"], "mean_latency_ms": s["performance"]["mean_latency_ms"],"median_latency_ms":s["performance"].get("median_latency_ms"),"p95_latency_ms":s["performance"].get("p95_latency_ms"), "theses": s["instrument_quality"]["proposed"], "confirmations": s["execution"]["confirmation_candidates"],"safety_failures":s["engineering"].get("safety_failures"),"lifecycle_failures":s["engineering"].get("lifecycle_failures"),"unexpected_orders":s["engineering"].get("unexpected_order_attempts")})
