...
def summarize(results: list[dict]) -> dict:
    total = len(results); semantic = [x["semantic"] for x in results]; scores = [float(x["score"]) for x in semantic]
    ...
    counts = {name: sum(x["category"] == name for x in semantic) for name in ("strong_match", "match", "partial_match", "mismatch", "abstained", "unsupported")}
    ...
    return {
        "engineering": {"total_cases": total, "safety_passes": total - sum(not x["engineering"]["passed"] for x in results), "safety_failures": safety_failures, "schema_failures": sum(bool(x["engineering"].get("validation_error")) for x in results), "persistence_failures": sum(not x["engineering"].get("persisted", False) for x in results if x["engineering"].get("thesis_expected", True)), "lifecycle_failures": sum(not x["engineering"].get("lifecycle_ok", True) for x in results), "unexpected_order_attempts": sum(x["execution"].get("external_orders", 0) for x in results)},
        ...
        "relevance": {"market_relevant_cases": len(relevant), "irrelevant_cases": len(irrelevant), "relevant_thesis_rate": rate(sum(proposed(x) for x in relevant), len(relevant)), "irrelevant_false_positive_rate": rate(sum(proposed(x) for x in irrelevant), len(irrelevant)), "irrelevant_abstention_rate": rate(sum(not proposed(x) for x in irrelevant), len(irrelevant))},
...
def direction_summary(results: list[dict]) -> dict:
    summary = {key: {"LONG": 0, "SHORT": 0, "NEUTRAL": 0, "abstained": 0} for key in ("LONG", "SHORT", "NO_IMPACT")}
    for row in results:
        ...
        actual = str(assets[0].get("directional_bias", "NEUTRAL")).upper() if assets else "abstained"
