...
def summarize(results: list[dict]) -> dict:
    ...
    consistency_checked=sum(x["ai"].get("direction_consistency",{}).get("checked",0) for x in results);consistency_ok=sum(x["ai"].get("direction_consistency",{}).get("consistent",0) for x in results)
    lifecycle_states = {}
    ...
    return {
        ...
        "direction": {**direction_summary(results),"causal_consistency":{"checked":consistency_checked,"consistent":consistency_ok,"rate":rate(consistency_ok,consistency_checked)}},
        "performance": {"ai_requests": total,"transport_successes":sum(x["ai"].get("transport_status")=="SUCCESS" for x in results),"transport_errors":sum(x["ai"].get("transport_status")=="ERROR" and x["ai"].get("status")!="TIMEOUT" for x in results), "successful_responses":sum(x["engineering"].get("schema_status")=="PASS" or consistency_failed(x) for x in results),"invalid_responses":sum(x["engineering"].get("schema_status")=="FAIL" and not consistency_failed(x) for x in results), "errors":sum(x["engineering"].get("schema_status")!="PASS" for x in results), "timeouts": sum(x["ai"].get("status")=="TIMEOUT" for x in results),"requests_reconciled":sum(x["engineering"].get("schema_status") in {"PASS","FAIL","NOT_EVALUATED"} for x in results)==total, "mean_latency_ms": round(mean(latencies), 3) if latencies else None, "median_latency_ms": round(median(latencies), 3) if latencies else None, "p90_latency_ms":percentile(latencies,.90), "p95_latency_ms": percentile(latencies, .95),"completion_tokens":sum((x["timings"].get("usage") or {}).get("completion_tokens",0) for x in results),"reasoning_tokens":sum(((x["timings"].get("usage") or {}).get("completion_tokens_details") or {}).get("reasoning_tokens",0) for x in results)},
