...
class Trade:
 ...
 def close(self,time:datetime,price:float,reason:ExitReason,costs:float=0):
  self.exit_time=time;self.average_exit=price;self.exit_reason=reason;self.gross_pnl=(price-self.average_entry)*self.quantity*(1 if self.direction=="long" else -1);self.net_pnl=self.gross_pnl-costs

class TradeLifecycle:
 def __init__(self):self.open:dict[str,Trade]={};self.completed:list[Trade]=[]
 def start(self,instrument,direction,strategy,version,signal_id,time,quantity,price):
  trade=Trade(str(uuid4()),instrument,direction,strategy,version,signal_id,time,quantity,price);self.open[instrument]=trade;return trade
 def update(self,instrument,bid,ask):
  if instrument in self.open:self.open[instrument].update_excursion(bid,ask)
 def close(self,instrument,time,price,reason,costs=0):
  trade=self.open.pop(instrument);trade.close(time,price,reason,costs);self.completed.append(trade);return trade
